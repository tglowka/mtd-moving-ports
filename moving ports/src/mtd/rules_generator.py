...
class RulesGenerator:
    def __init__(self, mtd_controller_configuration: MtdControllerConfiguration) -> None:
        self.__mtd_controller_configuration = mtd_controller_configuration

        self.__max_port_number = self.__mtd_controller_configuration.get_max_port_numer()
        self.__all_used_ports_by_protocol = self.__mtd_controller_configuration.get_all_used_ports_by_protocol()
        self.__ports_to_shuffle_by_address_and_protocol = self.__mtd_controller_configuration.get_ports_to_shuffle_by_address_and_protocol()
        self.__nft_startup_template = self.__mtd_controller_configuration.get_nft_startup_file_content_as_string()
        self.__nft_address_rules_template = self.__mtd_controller_configuration.get_nft_address_rules_content_as_string()
    ...
    def __choose_closed_ports(self, count, protocol) -> Set[int]:
        closed_ports = set()
        ...
        while(len(closed_ports) < count):
            ...
            if (self.__check_all_used_ports_contains(protocol=protocol, port_number=port_number)):
                continue
    ...
    def __check_all_used_ports_contains(self, protocol: str, port_number: int) -> bool:
        contains = (protocol in self.__all_used_ports_by_protocol and
                    port_number in self.__all_used_ports_by_protocol[protocol])
        return contains

    def __get_template_with_replaced_tokens(self, source_address: str, destination_port: int, redirected_port: int, protocol: str) -> str:
        rules = ""
